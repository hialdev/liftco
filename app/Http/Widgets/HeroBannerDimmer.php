...
class HeroBannerDimmer extends BaseDimmer
{
    /**
    ...
    public function run()
    {
        ...
        return view('admin.dimmer', array_merge($this->config, [
            'icon'   => 'voyager-images',
            'title'  => "{$count} {$string}",
            'text'   => 'Jumbotron / Hero Banner',
            'button' => [
                'text' => 'Kelola Hero Banners',
                'link' => route('voyager.hero-banner.index'),
            ],
    ...
    {
        return Auth::user()->can('browse', app(HeroBanner::class));
    }
}